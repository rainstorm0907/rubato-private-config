...
2. `untried_routes`가 **빈 배열**이어야 함. 비어있지 않으면 그 경로들을 먼저 실행.
...
| 태그 | 실행기 | 언제 |
|------|--------|------|
| `needs_protocol_stealth` | `protocol_stealth_chrome` (nodriver → patchright+channel=chrome) | 자동화 프로토콜(Runtime.enable)을 지문화하는 게이트다. Playwright 심 계열은 패치 무관 실패(2026 벤치 실측) |
...
실패(`ok=False`) 응답에는 `block_class`가 붙는다. `bot_detection`(라우트 결과가 엇갈리거나 WAF 시그널 → 브라우저·다른 라우트로 우회 가능) vs `infra_or_auth`(모든 라우트가 균일하게 401/404 → 스텔스로 우회 불가) 형태이며, 재시도 가치 판단에 사용한다.
...
# X 프로필 타임라인 (rate-limit 변동 시 engine이 재시도) / 키워드: WebSearch(site:x.com {kw})→tweet-result
...
> 커버리지 회귀 점검: `python3 tests/coverage_battery.py`로 플랫폼별 전수 경로 pass/fail + 썩은 예시 자동 적발.
