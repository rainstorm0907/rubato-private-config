...
| Parallel spawn | `subagent.create` returns a handle without waiting, so several children can be in flight at once; wait with `inspect.wait`, never a sleep loop. Same rule for the lead spawning teammates and a teammate running subagents |
