...
The return contract has a fixed column when frozen items were listed: `Frozen items touched: none` or `Frozen items touched: <which>, <why the task required it>`. A return without this column when the brief listed frozen items is incomplete. This column exists so that the worker checks the list before returning, and so that you can see a violation without reading the diff.
